...
echo ""
echo "════════════════════════════════════════════"
echo "  Building PAI ARM64 live ISO..."
echo "════════════════════════════════════════════"
echo ""
...
# Remove live-build download cache
...
# Remove Ollama GPU libs (CPU-only live USB)
...
# ── Raspberry Pi Imager artifacts ──────────────────────────────────────────
# The arm64 ISO built above is UEFI/grub-efi — it boots a Pi only via uboot
# shims. Raspberry Pi Imager consumes a raw .img.xz with a FAT32
# /boot/firmware + ext4 root partition layout. If a raw Pi image is present
# at ./pai-*-arm64.img next to this build, package it for dist/; otherwise
# log a pointer so gen-imager-manifest.mjs emits the pending manifest.
# See docs/src/advanced/building-from-source.md (§ "Raspberry Pi image").
RAW_IMG=$(ls ./pai-*-arm64.img 2>/dev/null | head -n1 || true)
if [[ -n "${RAW_IMG}" ]]; then
    echo "[pai] packaging Pi Imager artifacts from ${RAW_IMG}"
    ../scripts/package-arm64-img.sh "${RAW_IMG}" || \
        echo "[pai] WARN: package-arm64-img.sh failed, skipping Imager assets"
else
    echo "[pai] no raw Pi .img artifact present; Imager manifest will stay in 'pending' state"
    echo "[pai] (see docs/src/advanced/building-from-source.md § Raspberry Pi image)"
fi
