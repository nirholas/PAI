...
#   recommend-model.sh --shell         # eval-able KEY=VALUE pairs
...
USAGE
    recommend-model.sh [--name|--json|--shell|--explain|-h|--help]

OPTIONS
    --name       Print only the model name (for scripting)
    --json       Print structured JSON output
    --shell      Print eval-able PAI_* shell variable assignments
...
PAI_MODEL="${MODEL}"
PAI_TIER="${TIER}"
PAI_PULL_NEEDED=${PULL_NEEDED}
PAI_SIZE_NOTE="${SIZE_NOTE}"
PAI_RAM_MB=${RAM_MB}
PAI_RAM_GB=${RAM_GB}
PAI_GPU_VENDOR="${GPU_VENDOR}"
PAI_GPU_VRAM_MB=${GPU_VRAM_MB}
PAI_GPU_MODEL="${GPU_MODEL}"
PAI_EFFECTIVE_MB=${EFFECTIVE_MB}
PAI_EFFECTIVE_SOURCE="${EFFECTIVE_SOURCE}"
EOF
        ;;
    json)
        cat <<EOF
